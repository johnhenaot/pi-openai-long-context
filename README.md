...
To use an equivalent model through another provider, add its provider name to the same config file:
...
This applies to manual toggles, the command menu, and sessions opted in with either auto-enable flag. Omit the auto-enable flags if you only want manual toggles. Only add providers whose GPT-5.6 or GPT-6 model documents a context window of at least 1.05M tokens.
